/* ============================================================================
   DATA MANAGER MODULE - CJC Namespace Version
   ============================================================================ */

CJC.defineModule('dataManager', function() {
    'use strict';
    ...
    function saveDatabase() {
        try {
            ...
            const result = sqlDB.saveToLocalStorage(DATA_MANAGER_CONFIG.STORAGE_KEY);
            if (result.error) {
    ...
    function loadDatabase() {
        try {
            const result = sqlDB.loadFromLocalStorage(DATA_MANAGER_CONFIG.STORAGE_KEY);
            if (result.error) {
    ...
    function createBackup(name = null) {
        try {
            ...
            const { data: dbData, error } = sqlDB.exportDatabase();
            if (error) {
    ...
    function restoreFromBackup(backupIndex) {
        try {
            ...
            const result = sqlDB.importDatabase(dbData);
            if (result.error) {
                throw new Error(result.error);
            }
    ...
    function exportToFile(filename = null) {
        try {
            ...
            const { data, error } = sqlDB.exportDatabase();
            if (error) {
                throw new Error(error);
            }
            ...
            try {
                ...
                reader.onload = function(e) {
                    try {
                        ...
                        const result = sqlDB.importDatabase(data);
                        if (result.error) {
    ...
    function initialize() {
        console.log('✅ Data manager module loaded in CJC namespace');
    }
...
(function() {
    ...
    function setupCompatibility() {
        if (CJC && CJC.modules && CJC.modules.dataManager) {
            // Create global reference
            window.dataManager = CJC.modules.dataManager;
            
            console.log('✅ Data manager compatibility layer established');
        } else {
            // Retry in a moment
